...
| 年齡分級 | ⏳ 等業主 | 問卷答完（類型「所有其他應用程式類型」、9 題全「否」、實體媒體「否」），預覽：IARC 全域 **3+**、ESRB 所有人、PEGI 3+、USK 所有人、巴西／智利／俄羅斯所有年齡。🔴 儲存前要勾「同意 IARC 使用規定、已達成年年齡」——**同意條款＋本人聲明，AI 不代勾**，業主自己勾、按儲存 |
| 套件 | ⏳ 等業主 | 人工拖 `build\msix\AiUsageMeter-0.1.0.0-x64.msix`（47.2 MB；desk-pet 45 MB 就超過自動化上傳上限）。啟動回應那版 WACK PASS（13:55）；加版本資訊、改檔名後要再跑一次 |
| Store 清單 | ⏳ 等套件 | 「套件中支援的語言」要上傳套件才出現；**先不要手動加語言**（desk-pet 踩過 en-US／en 被當成兩個語言） |
| 提交選項 | ⏳ 等套件 | runFullTrust 說明欄要有套件才出現（§5）。發行方式維持預設「通過認證後立即發佈」 |
