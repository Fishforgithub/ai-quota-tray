...
- **霓虹外圈會慢慢旋轉**：鎖定畫面、螢幕關閉、省電模式或 Windows 關掉「動畫效果」時會自動停下。
- **有新版本時提醒你**：Microsoft Store 版會跳通知，設定視窗也會出現直通 Store 的按鈕。
...
AI Usage Meter **不讀取、不保存你的密碼或登入權杖**。需要即時數字時，它請你電腦上已經登入的官方工具自己去查，再把結果顯示出來。平常在背景只讀本機紀錄，只有在你打開卡片時才會查詢雲端，而且幾分鐘內不重複查（Microsoft Store 版另外會定期向 Store 詢問有沒有新版本）。

## 隱私

**沒有自己的伺服器、沒有分析追蹤、沒有第三方廣告，也不收集個人資料。**我們收不到你的任何資料。
